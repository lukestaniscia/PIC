...
When reproducing the results in the article, first download the PDB files for the smallest 10 proteins in terms of atom size and the mmCIF files for the largest 10 proteins from the Protein Data Bank (https://www.rcsb.org). Protein files can be downloaded from its webpage by searching the protein ID in the search bar on the top of the home page. Place each file in its own folder and name the folder the protein's ID in lowercase. Place all 20 folders in one in your working directory or in a directory downstream. 
